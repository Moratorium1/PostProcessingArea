...
std::vector<PostProcess> gPolygonPostProcess = { PostProcess::PolygonGradient, PostProcess::PolygonWater, PostProcess::PolygonHoriGaussian, PostProcess::PolygonVertGaussian, PostProcess::PolygonDistort };
...
ID3D11ShaderResourceView* gSceneTextureSRV2		= nullptr;
...
bool InitGeometry()
{
	...
	if (FAILED(gD3DDevice->CreateTexture2D(&sceneTextureDesc, NULL, &gSceneTexture)) || FAILED(gD3DDevice->CreateTexture2D(&sceneTextureDesc, NULL, &gSceneTexture2)))
	{
	...
	if (FAILED(gD3DDevice->CreateRenderTargetView(gSceneTexture, NULL, &gSceneRenderTarget)) || FAILED(gD3DDevice->CreateRenderTargetView(gSceneTexture2, NULL, &gSceneRenderTarget2)))
	{
	...
	if (FAILED(gD3DDevice->CreateShaderResourceView(gSceneTexture, &srDesc, &gSceneTextureSRV)) || FAILED(gD3DDevice->CreateShaderResourceView(gSceneTexture2, &srDesc, &gSceneTextureSRV2)))
	{
...
bool InitScene()
{
	...
	gWall->SetPosition({ 20, -0.5, 10 });
	gWall->SetRotation({ 0.0f, ToRadians(0.0f), 0.0f });
	gWall->SetScale(35.0f);
...
void SelectPostProcessShaderAndTextures(PostProcess postProcess)
{
	...
	else if (postProcess == PostProcess::VerticalColourGradient || postProcess == PostProcess::PolygonGradient)
	{
		gD3DContext->PSSetShader(gVerticalColourGradientPostProcess, nullptr, 0);
	}
	...
	else if (postProcess == PostProcess::Water || postProcess == PostProcess::PolygonWater)
	{
	...
	else if (postProcess == PostProcess::VertGaussian || postProcess == PostProcess::PolygonVertGaussian)
	{
		gD3DContext->PSSetShader(gVertGaussianPostProcess, nullptr, 0);
	}

	else if (postProcess == PostProcess::HoriGaussian || postProcess == PostProcess::PolygonHoriGaussian)
	{
		gD3DContext->PSSetShader(gHorizGaussianPostProcess, nullptr, 0);
	}
	...
	else if (postProcess == PostProcess::Distort || postProcess == PostProcess::PolygonDistort)
	{
...
void RenderScene()
{
	...
	if (gCurrentPostProcess != PostProcess::None)
	{
		const int numProcesses = gPostProcesses.size() - 1;
		int count = 0;
		bool bIsOdd = false;
		for (auto& Process : gPostProcesses)
		{
			if (!bIsOdd)
			{
				gD3DContext->OMSetRenderTargets(1, &gSceneRenderTarget2, gDepthStencil);
				gD3DContext->PSSetShaderResources(0, 1, &gSceneTextureSRV);
			}
			else
			{
				gD3DContext->OMSetRenderTargets(1, &gSceneRenderTarget, gDepthStencil);
				gD3DContext->PSSetShaderResources(0, 1, &gSceneTextureSRV2);
			}

			if (count == numProcesses)	gD3DContext->OMSetRenderTargets(1, &gBackBufferRenderTarget, gDepthStencil);

			/*Check to see if the process is a polygon post process */
			bool bIsPolygon = false;
			for (auto& polygonProcesses : gPolygonPostProcess)
			{
				if (Process == polygonProcesses)
					bIsPolygon = true;
			}

			/* if it is not a polygon post process carryout fullscreen else use PolygonPostProcessing */
			if (!bIsPolygon)
			{
			...
			else
			{
				...
				static CMatrix4x4 polyMatrix = MatrixTranslation(gWall->Position() - CVector3{ 0, -8, 0 });
...
void UpdateScene(float frameTime)
{
	...
	if (KeyHit(Key_0))
	{
		...
		gPostProcessingConstants.hueTimer = 0;
		gPostProcesses.clear();
	}

	if (KeyHit(Key_1))
	{
		gCurrentPostProcess = PostProcess::VerticalColourGradient;
		gPostProcesses.push_back(gCurrentPostProcess);
	}

	if (KeyHit(Key_2))
	{
		gCurrentPostProcess = PostProcess::VertGaussian;
		gPostProcesses.push_back(gCurrentPostProcess);

		gCurrentPostProcess = PostProcess::HoriGaussian;
		gPostProcesses.push_back(gCurrentPostProcess);
	}
	...
	/*PostProcess::PolygonGradient, PostProcess::PolygonWater, PostProcess::PolygonHoriGaussian, PostProcess::PolygonVertGaussian, PostProcess::PolygonDistort */

	if (KeyHit(Key_6))
	{
		gCurrentPostProcess = PostProcess::PolygonGradient;
		gPostProcesses.push_back(gCurrentPostProcess);
	}

	if (KeyHit(Key_7))
	{
		gCurrentPostProcess = PostProcess::PolygonWater;
		gPostProcesses.push_back(gCurrentPostProcess);
	}

	if (KeyHit(Key_8))
	{
		gCurrentPostProcess = PostProcess::PolygonVertGaussian;
		gPostProcesses.push_back(gCurrentPostProcess);

		gCurrentPostProcess = PostProcess::PolygonHoriGaussian;
		gPostProcesses.push_back(gCurrentPostProcess);
	}

	if (KeyHit(Key_9))
	{
		gCurrentPostProcess = PostProcess::PolygonDistort;
		gPostProcesses.push_back(gCurrentPostProcess);
	}
	...
	gPostProcessingConstants.gradientColour1 = { 1, 0, 0 };
	gPostProcessingConstants.gradientColour2 = { 0, 0, 1 };
	...
	// Update water timer
	gPostProcessingConstants.waterTimer += frameTime;

	// Update water timer
	bool bShiftHue = false;
	for (auto& process : gPostProcesses)
		if (process == PostProcess::VerticalColourGradient)
			bShiftHue = true;

	if (bShiftHue)
		gPostProcessingConstants.hueTimer += frameTime * 2.5f;
