...
ID3D11PixelShader*  gVerticalColourGradientPostProcess = nullptr;
ID3D11PixelShader*  gBlurPostProcess	   = nullptr;
ID3D11PixelShader*  gWaterPostProcess	   = nullptr;

ID3D11PixelShader* gVertGaussianPostProcess;
ID3D11PixelShader* gHorizGaussianPostProcess;
...
bool LoadShaders()
{
	...
	gVerticalColourGradientPostProcess = LoadPixelShader("VerticalColourGradient_pp");
	gBlurPostProcess	   = LoadPixelShader("Blur_pp");
	gWaterPostProcess	   = LoadPixelShader("UnderWater_pp");
	gTintPostProcess       = LoadPixelShader ("Tint_pp");

	gVertGaussianPostProcess = LoadPixelShader("VerticalGaussianBlur_pp");
	gHorizGaussianPostProcess = LoadPixelShader("HorizontalGaussianBlur_pp");
	...
	if (gBasicTransformVertexShader == nullptr || gPixelLightingVertexShader == nullptr ||
		...
		g2DPolygonVertexShader      == nullptr || gVerticalColourGradientPostProcess == nullptr ||
		gBlurPostProcess			== nullptr || gWaterPostProcess == nullptr ||
		gVertGaussianPostProcess == nullptr || gHorizGaussianPostProcess == nullptr)
	{
...
void ReleaseShaders()
{
	...
	if (gVerticalColourGradientPostProcess) gVerticalColourGradientPostProcess->Release();
	if (gBlurPostProcess)			  gBlurPostProcess->Release();
	if (gWaterPostProcess)			  gWaterPostProcess->Release();

	if (gVertGaussianPostProcess)			  gVertGaussianPostProcess->Release();
	if (gHorizGaussianPostProcess)			  gHorizGaussianPostProcess->Release();
